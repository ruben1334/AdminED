    ...
    if ($_POST['idMaestro'] != null) {
      ...
      foreach ($_POST['idarticulo'] as $key => $id) {
        $cantidad = $_POST['cantidad'][$key];
         ...
         $razonSocial = $_POST['nombreClase'][$key];
 ...
 $lista = $this->pedidodetalle_model-> recuperarPedidoDetalle($idPedido);
  ...
  //  $anulado   = base_url() . "fotos/anulado.png";
   $titulo   = ('Reporte de Pedidos');
    ...
    $x1        = 35;
    $y1        = 10;
    $this->pdf->Image($logo, 35, 55, 25, 23);
    $this->pdf->Image($logo1,120, 45, 40, 45);
    //if ($data['estado'] == 0) {
   ...
   $r2 = $r1 + 50;
    ...
    $this->pdf->Cell($r2 - $r1 - 1, 10, 'Fecha. ' . $fecha=$row->fecha, 0, 2, "C");
   // $this->pdf->Cell($r2 - $r1 - 1, 5, 'No. ' . $pedido['nroComprobante'], 1, 2, "C");
    ...
    $this->pdf->SetFont('Arial', 'B', 10);
    $this->pdf->Cell(10, 6, utf8_decode('Nº'), 1, 0, 'C', 1);
    $this->pdf->Cell(40, 6, utf8_decode('DESTINADO AL AULA'), 1, 0, 'C', 1);
    $this->pdf->Cell(50, 6, utf8_decode('NOMBRE DEL MATERIAL'), 1, 0, 'C', 1);
    $this->pdf->Cell(20, 6, utf8_decode('CANTIDAD'), 1, 0, 'C', 1);
...
$nombreMaterial=$row->nombreMaterial;
 $razonSocial=$row->razonSocial;
      $cantidad=$row->cantidad;
 $this->pdf->Cell(10, 6, $num,1,0,'C', 1);
 $this->pdf->Cell(40, 6, $razonSocial,1,0,'C', 1);
 $this->pdf->Cell(50, 6, $nombreMaterial,1,0,'C', 1);
$this->pdf->Cell(20, 6, $cantidad,1,1,'C', 1);
$num++;
...
$this->pdf->SetFont('Arial', 'B', 10);
  ...
  public function reportePedidos()
  {
    ...
    $this->pdf->Cell(10, 5, utf8_decode("No."), 'TBLR', 0, 'L', 1);
    $this->pdf->Cell(40, 5, utf8_decode("NRO. COMPROBANTE"), 'TBLR', 0, 'L', 1);
    ...
    foreach ($data as $row) {
      ...
      $this->pdf->Cell(40, 5, utf8_decode($row['nroComprobante']), 'TBLR', 0, 'L', 0);
      $this->pdf->Cell(50, 5, utf8_decode($row['fecha']), 'TBLR', 0, 'L', 1);
      $this->pdf->Cell(40, 5, utf8_decode($row['cantidad']), 'TBLR', 0, 'C', 1);
      $this->pdf->Ln(5);
